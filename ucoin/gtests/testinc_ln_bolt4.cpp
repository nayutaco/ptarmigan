...
TEST_F(onion, testvector)
{
    ...
    ln_self_t self;
    bool ret = ln_onion_create_packet(&self, packet, hop_datain, 5, SESSIONKEY, ASSOC, sizeof(ASSOC));
    ASSERT_TRUE(ret);
...
TEST_F(onion, test1)
{
    ...
    ln_self_t self;
    bool ret = ln_onion_create_packet(&self, packet, datain, 1, session_key, NULL, 0);
    ASSERT_TRUE(ret);
    ...
    ret = ln_onion_read_packet(packet, &dataout, PACKET, onion_privkey[0], NULL, 0);
    ASSERT_TRUE(ret);
...
TEST_F(onion, test2)
{
    ...
    ln_self_t self;
    bool ret = ln_onion_create_packet(&self, packet, datain, 20, session_key, NULL, 0);
    ASSERT_TRUE(ret);
    ...
    for (int lp = 0; lp < 20; lp++) {
        ret = ln_onion_read_packet(packet, &dataout, packet, onion_privkey[lp], NULL, 0);
        ASSERT_TRUE(ret);
...
TEST_F(onion, test4)
{
    ...
    ln_self_t self;
    bool ret = ln_onion_create_packet(&self, packet, datain, 20, session_key, ASSOC, sizeof(ASSOC));
    ASSERT_TRUE(ret);

    ln_hop_dataout_t dataout;

    for (int lp = 0; lp < 20; lp++) {
        ret = ln_onion_read_packet(packet, &dataout, packet, onion_privkey[lp], ASSOC, sizeof(ASSOC));
        ASSERT_TRUE(ret);
        ASSERT_EQ(datain[lp].short_channel_id, dataout.short_channel_id);
