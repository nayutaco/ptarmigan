...
TEST_F(keys, keys_is_valid_priv1)
{
    ...
    bool ret = ucoin_keys_chkpriv(PRIV);
    ASSERT_FALSE(ret);
}
...
TEST_F(keys, keys_1)
{
    ...
    ret = ucoin_keys_priv2pub(pub, priv);
    ASSERT_TRUE(ret);
    ASSERT_EQ(0, memcmp(PUB, pub, sizeof(PUB)));

    ASSERT_TRUE(ucoin_keys_chkpub(pub));

    ret = ucoin_keys_pubuncomp(uncomppub, pub);
    ASSERT_TRUE(ret);
    ...
    ASSERT_EQ(UCOIN_PREF_P2PKH, pref);
}
...
TEST_F(keys, pub2uncomp)
{
    ...
    for (int lp = 0; lp < 4; lp++) {
        bool ret = ucoin_keys_pubuncomp(uncomppub, COMP[lp]);
        ASSERT_TRUE(ret);
...
TEST_F(keys, multi_2of2_1)
{
    ...
    bool ret;
    ucoin_buf_t bufredeem;
    ret = ucoin_keys_create2of2(&bufredeem, PUB1, PUB2);
...
TEST_F(keys, multi_2of2_2)
{
    ...
    bool ret;
    ucoin_buf_t bufredeem;
    ret = ucoin_keys_create2of2(&bufredeem, PUB2, PUB1);
    //keys::DumpBin(bufredeem.buf, bufredeem.len);
    ASSERT_TRUE(ret);
    ASSERT_EQ(sizeof(REDEEM), bufredeem.len);
    ASSERT_EQ(0, memcmp(REDEEM, bufredeem.buf, sizeof(REDEEM)));
...
TEST_F(keys, multi_2of2_same)
{
    ...
    bool ret;
    ucoin_buf_t bufredeem;
    ret = ucoin_keys_create2of2(&bufredeem, PUB1, PUB1);
    ASSERT_FALSE(ret);
    //ucoin_buf_free(&bufredeem);
}
#endif
...
TEST_F(keys, multi_2of3)
{
    ...
    ucoin_buf_t bufredeem;
    bool ret = ucoin_keys_createmulti(&bufredeem, PUBS, 3, 2);
    //keys::DumpBin(bufredeem.buf, bufredeem.len);
    ASSERT_TRUE(ret);
    ASSERT_EQ(sizeof(REDEEM), bufredeem.len);
    ASSERT_EQ(0, memcmp(REDEEM, bufredeem.buf, sizeof(REDEEM)));
...
TEST_F(keys, uncomp1)
{
    ...
    uint8_t uncomp[UCOIN_SZ_PUBKEY_UNCOMP];

    mbedtls_ecp_point_init(&unpnt);
    int ret = ucoin_util_ecp_point_read_binary2(&unpnt, COMP);
    ASSERT_EQ(0, ret);
