...
#define UCOIN_SZ_PUBKEY     (33)


static uint64_t ln_calc_obscured_txnum(const uint8_t *pLocalBasePt, const uint8_t *pRemoteBasePt)
{
    uint64_t obs = 0;
    uint8_t base[32];
...
int main(void)
{
    ...
    uint64_t obscured = ln_calc_obscured_txnum(OPEN_CH_PAYMENT_BP, ACCEPT_CH_PAYMENT_BP);
