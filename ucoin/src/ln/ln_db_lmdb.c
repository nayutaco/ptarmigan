...
bool ln_db_self_del(const ln_self_t *self, void *p_db_param)
{
    ...
    retval = mdb_dbi_open(p_cur->txn, LNDBI_ANNO_CNL, 0, &dbi_anno);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
...
bool ln_db_anno_cur_transaction(void **ppDb, ln_db_txn_t Type)
{
    ...
    if (retval == 0) {
        ...
        *ppDb = p_db;
        ...
        switch (Type) {
        case LN_DB_TXN_CNL:
            p_name = LNDBI_ANNOINFO_CNL;
            break;
        case LN_DB_TXN_NODE:
            p_name = LNDBI_ANNOINFO_NODE;
            break;
        default:
            assert(0);
            return false;
...
bool ln_db_annocnl_load(ucoin_buf_t *pCnlAnno, uint64_t ShortChannelId)
{
    ...
    retval = mdb_dbi_open(db.txn, LNDBI_ANNO_CNL, 0, &db.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
...
bool ln_db_annocnl_save(const ucoin_buf_t *pCnlAnno, uint64_t ShortChannelId, const uint8_t *pSendId)
{
    ...
    db_info.txn = db.txn;
    retval = mdb_dbi_open(db.txn, LNDBI_ANNO_CNL, MDB_CREATE, &db.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
    }
    retval = mdb_dbi_open(db.txn, LNDBI_ANNOINFO_CNL, MDB_CREATE, &db_info.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
...
bool ln_db_annocnlall_del(uint64_t ShortChannelId)
{
    ...
    retval = mdb_dbi_open(txn, LNDBI_ANNO_CNL, MDB_CREATE, &dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_ABORT;
    }
    retval = mdb_dbi_open(txn, LNDBI_ANNOINFO_CNL, MDB_CREATE, &dbi_info);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_ABORT;
    }
...
bool ln_db_annonod_load(ucoin_buf_t *pNodeAnno, uint32_t *pTimeStamp, const uint8_t *pNodeId)
{
    ...
    retval = mdb_dbi_open(db.txn, LNDBI_ANNO_NODE, 0, &db.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
...
bool ln_db_annonod_save(const ucoin_buf_t *pNodeAnno, const ln_node_announce_t *pAnno, const uint8_t *pSendId)
{
    ...
    db_info.txn = db.txn;
    retval = mdb_dbi_open(db.txn, LNDBI_ANNO_NODE, MDB_CREATE, &db.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
    }
    retval = mdb_dbi_open(db.txn, LNDBI_ANNOINFO_NODE, MDB_CREATE, &db_info.dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        MDB_TXN_ABORT(db.txn);
        goto LABEL_EXIT;
...
bool ln_db_preimg_cur_open(void **ppCur)
{
    ...
    retval = mdb_dbi_open(p_cur->txn, LNDBI_PREIMAGE, 0, &p_cur->dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
bool ln_db_phash_save(const uint8_t *pPayHash, const uint8_t *pVout, ln_htlctype_t Type, uint32_t Expiry, void *pDbParam)
{
    ...
    retval = mdb_dbi_open(txn, LNDBI_PAYHASH, MDB_CREATE, &dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
bool ln_db_phash_search(uint8_t *pPayHash, ln_htlctype_t *pType, uint32_t *pExpiry, const uint8_t *pVout, void *pDbParam)
{
    ...
    retval = mdb_dbi_open(txn, LNDBI_PAYHASH, 0, &dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
int ln_lmdb_ver_check(ln_lmdb_db_t *pDb, uint8_t *pMyNodeId, ucoin_genesis_t *pGType)
{
    ...
    retval = mdb_dbi_open(pDb->txn, LNDBI_VERSION, 0, &pDb->dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
ln_lmdb_dbtype_t ln_lmdb_get_dbtype(const char *pDbName)
{
    ...
    } else if (strcmp(pDbName, LNDBI_ANNO_CNL) == 0) {
        //channel_announcement
        dbtype = LN_LMDB_DBTYPE_CHANNEL_ANNO;
    } else if (strcmp(pDbName, LNDBI_ANNO_NODE) == 0) {
        //node_announcement
        dbtype = LN_LMDB_DBTYPE_NODE_ANNO;
    } else if (strcmp(pDbName, LNDBI_ANNOINFO_CNL) == 0) {
        //channel_announcement/channel_update information
        dbtype = LN_LMDB_DBTYPE_NODE_ANNOINFO;
    } else if (strcmp(pDbName, LNDBI_ANNOINFO_NODE) == 0) {
        //node_announcement information
        dbtype = LN_LMDB_DBTYPE_NODE_ANNOINFO;
    } else if (strcmp(pDbName, LNDBI_PREIMAGE) == 0) {
        //preimage
        dbtype = LN_LMDB_DBTYPE_PREIMAGE;
#ifdef LN_UGLY_NORMAL
    } else if (strcmp(pDbName, LNDBI_PAYHASH) == 0) {
        //preimage
        dbtype = LN_LMDB_DBTYPE_PAYHASH;
#endif //LN_UGLY_NORMAL
    } else if (strcmp(pDbName, LNDBI_VERSION) == 0) {
        //version
...
static bool annocnl_cur_open(lmdb_cursor_t *pCur)
{
    ...
    retval = mdb_dbi_open(pCur->txn, LNDBI_ANNO_CNL, 0, &pCur->dbi);
    if (retval != 0) {
        //DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
static bool annonod_cur_open(lmdb_cursor_t *pCur)
{
    ...
    retval = mdb_dbi_open(pCur->txn, LNDBI_ANNO_NODE, 0, &pCur->dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
...
static bool preimg_open(ln_lmdb_db_t *p_db, MDB_txn *txn)
{
    ...
    retval = mdb_dbi_open(p_db->txn, LNDBI_PREIMAGE, MDB_CREATE, &p_db->dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
...
static int ver_write(MDB_txn *txn, const char *pWif, const char *pNodeName, uint16_t Port)
{
    ...
    retval = mdb_dbi_open(txn, LNDBI_VERSION, MDB_CREATE, &dbi);
    if (retval != 0) {
        DBG_PRINTF("err: %s\n", mdb_strerror(retval));
        goto LABEL_EXIT;
    }
