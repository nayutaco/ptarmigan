...
DUMPBIN(lntx_commit.remote.pubkey, UCOIN_SZ_PUBKEY);
    ...
    ret = ln_cmt_create(&tx_local, &buf_sig, &lntx_commit, true);
    if (!ret) {
                    ...
                    uint64_t dust_limit_sat)
{
    ...
    bool ret = ln_cmt_create(&tx_remote, &buf_sig, &lntx_commit, false);
    if (!ret) {
